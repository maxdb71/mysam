...
export default function IssueOrReceiptCreateUpdatePost(context) {
    ...
    binding.TempLine_Batch = libCom.getControlProxy(context,'BatchSimple').getValue();
    ...
    if (autoSerialNumberSwitch) {
        ...
        //let serialNumbers = [];
        //let picketValues = libCom.getControlProxy(context,'SerialNumListPicker').getValue();
        // if (picketValues.length > 0) {
        //     for (let index = 0; index < picketValues.length; index++) {
        //         serialNumbers[index] = picketValues[index].ReturnValue;
        //     }
        // }
        //binding.TempLine_SerialNumbers = serialNumbers;
        let serialNumbers = libCom.getControlProxy(context,'SerialNumListText').getValue();
        let serialNumbersArray = []; 
        if (serialNumbers.length > 0) {
            if (serialNumbers.indexOf(',') > 0) {
                binding.TempLine_SerialNumbers = serialNumbers.split(',');
            } else {
                serialNumbersArray.push(serialNumbers);
                binding.TempLine_SerialNumbers = serialNumbersArray;
            }
        } else {
            binding.TempLine_SerialNumbers = serialNumbersArray;
        }
    ...
    if (type === 'MaterialDocItem') {
        ...
        binding.TempLine_ToBatch = libCom.getControlProxy(context,'BatchNumTo').getValue();
