...
export default function WorkOrderChangeStatusPopover(context) {
     ...
     */
     let startedCheck = libWOMobile.isAnyWorkOrderStarted(context);
    /**
        ...
        if (!anythingStarted && context.binding.OrderMobileStatus_Nav.MobileStatus === STARTED && (cicoEnabled && !isClockedIn)) {
            ...
            return libSuper.checkReviewRequired(context, context.binding).then(review => {
                ...
                return context.read('/SAPAssetManager/Services/AssetManager.service', `${context.binding['@odata.readLink']}/OrderMobileStatus_Nav`, [], '$expand=OverallStatusCfg_Nav').then(rollback => {
                    ...
                    return context.read('/SAPAssetManager/Services/AssetManager.service', `${context.binding['@odata.readLink']}/OrderMobileStatus_Nav/OverallStatusCfg_Nav/OverallStatusSeq_Nav`, [], '$expand=NextOverallStatusCfg_Nav').then(codes => {
                        let popoverItems = [];
                        ...
                        codes.forEach(element => {
                            ...
                            } else if (statusElement.MobileStatus === COMPLETE && element.RoleType === supervisorRole) {
                                if (supervisorRole === 'S' || (supervisorRole === 'T' && !review)) { //Allow complete if supervisor or if technician and WO does not require review
