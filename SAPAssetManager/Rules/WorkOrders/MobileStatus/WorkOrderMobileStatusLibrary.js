...
export default class {
            ...
            return context.executeAction({'Name': '/SAPAssetManager/Actions/WorkOrders/MobileStatus/WorkOrderHoldUpdate.action', 'Properties': {
                ...
                return context.executeAction({'Name': '/SAPAssetManager/Actions/ClockInClockOut/WorkOrderClockInOut.action', 'Properties': {
                    'Properties': {
                        ...
                        'SubOperationNo': '',
                        'OrderId': binding.OrderId,
                        'PreferenceGroup': libCommon.getAppParam(context,'CICO','Enable') === 'Y' ? 'CLOCK_OUT' : 'END_TIME',
    ...
    static headerMobileStatus(context, binding) {
        ...
        if (libMobile.isHeaderStatusChangeable(context)) {
            return Promise.resolve(libMobile.getMobileStatus(binding, context));
        } else if (libMobile.isOperationStatusChangeable(context)) {
            return libOprMobile.operationRollUpMobileStatus(context, binding).catch(() => {
                return context.read('/SAPAssetManager/Services/AssetManager.service', `${binding['@odata.readLink']}/OrderMobileStatus_Nav/OverallStatusCfg_Nav`, [], '$select=OverallStatusLabel').then(result => {
                    return result.getItem(0).OverallStatusLabel;
                });
            });
        } else if (libMobile.isSubOperationStatusChangeable(context)) {
            return libOprMobile.operationRollUpMobileStatus(context, binding);
        }
        return Promise.resolve(libMobile.getMobileStatus(binding, context));
    }
