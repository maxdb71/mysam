...
export default function RequiredFields(context) {
    ...
    //Custom check for ENEL
    if ((function() { try { return context.evaluateTargetPathForAPI('#Control:CodingGroupLstPkr').visible; } catch (exc) { return false; } } )()) {
        required.push('CodingGroupLstPkr');
    }

      //Custom check for ENEL  
    if ((function() { try { return context.evaluateTargetPathForAPI('#Control:CodeDetailsLstPkr').visible; } catch (exc) { return false; } } )()) {
        required.push('CodeDetailsLstPkr');
    }
    ...
    if (causeDescription || causeGroup || causeCode) {
        ...
        required.push('CauseDescription', 'CauseGroupLstPkr','CodeLstPkr', 'ItemDescription', 'PartGroupLstPkr','PartDetailsLstPkr','DamageGroupLstPkr','DamageDetailsLstPkr');
