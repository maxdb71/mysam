...
export default function WorkCentreFilter(context) {

    var priorityObj = {name: 'Priority', values: []};
    var priorities = [];
   
    return context.read('/SAPAssetManager/Services/AssetManager.service', 'WorkCenters', [], '$orderby=Priority').then(function(data) {
        if (data.length > 0) {
